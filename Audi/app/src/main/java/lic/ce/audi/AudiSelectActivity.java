 ...
 */
public class AudiSelectActivity extends ListActivity implements LoaderManager.LoaderCallbacks<Cursor>
{
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_show_all_audio:
                ...
                refreshListView();
                return true;
            case R.id.action_open_S2T:
                Intent intent = new Intent();
                intent.setComponent(new ComponentName("lic.ce.s2t", "lic.ce.s2t.instances"));
    ...
    private void confirmDelete() {
        ...
        String artist = c.getString(c.getColumnIndexOrThrow(
                MediaStore.Audio.Media.ARTIST));
        CharSequence audiArtist =
                getResources().getText(R.string.artist_name);

        CharSequence message;
        if (artist.equals(audiArtist)) {
            message = getResources().getText(
                    R.string.confirm_delete_audi);
        } else {
            message = getResources().getText(
                    R.string.confirm_delete_non_audi);
        }
        ...
        new AlertDialog.Builder(AudiSelectActivity.this)
                .setTitle(title)
                .setMessage(message)
                .setPositiveButton(
    ...
    private void onRecord() {
        try {
            ...
            intent.setClassName( "lic.ce.audi", "lic.ce.audi.AudiEditActivity");
            startActivityForResult(intent, REQUEST_CODE_EDIT);
        } catch (Exception e) {
            Log.e("Audi", "Couldn't start editor");
        }
    }

    private void startAudiEditor() {
        Cursor c = mAdapter.getCursor();
        int dataIndex = c.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA);
        String filename = c.getString(dataIndex);
        try {
